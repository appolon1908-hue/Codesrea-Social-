...
SOCIAL_READ_SYNC_ENABLED = (
    os.getenv("SOCIAL_READ_SYNC_ENABLED", "false").lower() == "true"
)
BUSINESS_WRITES_ENABLED = (
    os.getenv("BUSINESS_WRITES_ENABLED", "false").lower() == "true"
)
...
async def ready(request: Request, session: AsyncSession = Depends(get_session)):
    ...
    if SOCIAL_READ_SYNC_ENABLED and not runtime_adapter.SOCIAL_RUNTIME_TOKEN:
        return JSONResponse(
            status_code=503,
            content={
                "status": "not_ready",
                ...
                "dependencies": {
                    "database": "ready",
                    "social_runtime_configuration": "unavailable",
                },
                "correlation_id": request.state.correlation_id,
            },
        )
...
def capabilities(request: Request = None) -> dict[str, object]:
    ...
    effective_publishing = SOCIAL_PUBLISHING_ENABLED and SOCIAL_PUBLISHING_AVAILABLE
    return {
        "service": SERVICE,
        "maintenance_mode": os.getenv("MAINTENANCE_MODE", "false").lower() == "true",
        "degraded_mode": False,
        "business_writes_enabled": BUSINESS_WRITES_ENABLED,
        "external_delivery_enabled": effective_publishing,
        "live_social_publish_enabled": effective_publishing,
        "read_only_mode": not BUSINESS_WRITES_ENABLED,
        "simulation_enabled": not effective_publishing,
...
async def create_post(
    body: PostCreate, session: AsyncSession = Depends(get_session)
) -> SocialPostModel:
...
async def runtime_post_snapshot(
    runtime_post_id: str, correlation_id: str | None = None
...
async def runtime_post_metrics(
    runtime_post_id: str, correlation_id: str | None = None
) -> dict[str, object] | None:
    if not SOCIAL_READ_SYNC_ENABLED:
        return None
