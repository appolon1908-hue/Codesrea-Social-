...
required=(PGHOST PGPORT PGDATABASE PGUSER PGPASSFILE CODESTRA_SOCIAL_BACKUP_ROOT CODESTRA_SOCIAL_RECOVERY_WORK_ROOT CODESTRA_RELEASE_SHA CODESTRA_IMAGE_DIGEST CODESTRA_BACKUP_GPG_RECIPIENT CODESTRA_BACKUP_GPG_SIGNING_FINGERPRINT)
...
backup_root=$CODESTRA_SOCIAL_BACKUP_ROOT
...
work=$(mktemp -d "$CODESTRA_SOCIAL_RECOVERY_WORK_ROOT/${stamp}.XXXXXX")
...
[[ "$database_name" == "codestra_social" || "$database_name" == "codestra_social_stage5" ]] || {
...
SCHEMA=codestra-social-backup.v1
STAMP=$stamp
