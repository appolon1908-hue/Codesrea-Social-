...
required=(PGHOST PGPORT PGDATABASE PGUSER PGPASSFILE CODESTRA_SOCIAL_BACKUP_DIR CODESTRA_SOCIAL_RESTORE_EVIDENCE_DIR CODESTRA_SOCIAL_RECOVERY_WORK_ROOT CODESTRA_EXPECTED_RELEASE_SHA CODESTRA_EXPECTED_IMAGE_DIGEST CODESTRA_BACKUP_GPG_SIGNING_FINGERPRINT)
for name in "${required[@]}"; do
  [[ -n "${!name:-}" ]] || { echo "required recovery setting is missing: $name" >&2; exit 2; }
done
backup_dir=$CODESTRA_SOCIAL_BACKUP_DIR
evidence_root=$CODESTRA_SOCIAL_RESTORE_EVIDENCE_DIR
...
[[ "$(metadata_value SCHEMA)" == "codestra-social-backup.v1" ]] || { echo "unsupported backup schema" >&2; exit 2; }
...
work=$(mktemp -d "$CODESTRA_SOCIAL_RECOVERY_WORK_ROOT/restore.XXXXXX")
...
and table_name in ('social_accounts','social_posts','social_metric_snapshots','social_sync_checkpoints');
SQL
)
...
with expected(table_name, columns, predicate) as (values
  ...
  ('social_sync_checkpoints', array['tenant_id','platform','external_account_id']::text[], null::text),
  ('social_posts', array['tenant_id','runtime_post_id']::text[], '(runtime_post_id IS NOT NULL)'::text)
)
select count(*) from expected e where exists (
