...
def _mock_tools(root: Path) -> Path:
    tools = root / "bin"
    tools.mkdir()
    _executable(tools / "psql", 'echo "${MOCK_PSQL_VALUE:-codestra_social}"\n')
...
def test_source_contract_is_encrypted_isolated_and_schema_aware():
    ...
    assert "CODESTRA_BACKUP_GPG_SIGNING_FINGERPRINT" in BACKUP + RESTORE
    ...
    for table in ("social_accounts", "social_posts", "social_metric_snapshots", "social_sync_checkpoints"):
    ...
    assert "(runtime_post_id IS NOT NULL)" in RESTORE
    ...
    assert "CODESTRA_EXPECTED_IMAGE_DIGEST" in RESTORE
...
def test_backup_publishes_verified_relocatable_artifacts():
    with tempfile.TemporaryDirectory() as directory:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "PATH": f"{tools}:{os.environ['PATH']}", "PGHOST": "synthetic.invalid", "PGPORT": "5432", "PGDATABASE": "codestra_social", "PGUSER": "synthetic", "PGPASSFILE": str(passfile), "CODESTRA_SOCIAL_BACKUP_ROOT": str(backup_root), "CODESTRA_SOCIAL_RECOVERY_WORK_ROOT": str(work_root), "CODESTRA_RELEASE_SHA": "1" * 40, "CODESTRA_IMAGE_DIGEST": "sha256:" + "2" * 64, "CODESTRA_BACKUP_GPG_RECIPIENT": "synthetic-test-recipient", "CODESTRA_BACKUP_GPG_SIGNING_FINGERPRINT": "A" * 40, "MOCK_SIGNER": "A" * 40},
...
def test_restore_refuses_source_identity_before_mutation():
    with tempfile.TemporaryDirectory() as directory:
        ...
        (backup / "METADATA").write_text("SCHEMA=codestra-social-backup.v1\nSTAMP=20260901T000000Z\nDATABASE=codestra_social\nRELEASE_SHA=" + "1" * 40 + "\nIMAGE_DIGEST=sha256:" + "2" * 64 + "\nSIGNING_FINGERPRINT=" + "A" * 40 + "\nENCRYPTION=OPENPGP\n")
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "PATH": f"{tools}:{os.environ['PATH']}", "PGHOST": "synthetic.invalid", "PGPORT": "5432", "PGDATABASE": "codestra_social", "PGUSER": "synthetic", "PGPASSFILE": str(passfile), "CODESTRA_SOCIAL_BACKUP_DIR": str(backup), "CODESTRA_SOCIAL_RESTORE_EVIDENCE_DIR": str(root / "evidence"), "CODESTRA_SOCIAL_RECOVERY_WORK_ROOT": str(work_root), "CODESTRA_EXPECTED_RELEASE_SHA": "1" * 40, "CODESTRA_EXPECTED_IMAGE_DIGEST": "sha256:" + "2" * 64, "CODESTRA_BACKUP_GPG_SIGNING_FINGERPRINT": "A" * 40, "ALLOW_ISOLATED_RESTORE": "true", "MOCK_PSQL_VALUE": "codestra_social", "MOCK_SIGNER": "A" * 40},
            text=True, capture_output=True, check=False,
        )
...
def test_freshness_passes_current_marker_and_fails_stale_marker():
    with tempfile.TemporaryDirectory() as directory:
        ...
        (artifact / "METADATA").write_text("SCHEMA=codestra-social-backup.v1\nSTAMP=" + current + "\nSIGNING_FINGERPRINT=" + "A" * 40 + "\n")
